...
func TestPersonFieldsService_List(t *testing.T) {
	t.Parallel()
	...
	fields, next, err := client.PersonFields.List(
		context.Background(),
		WithPersonFieldsIncludeFields(FieldIncludeField("ui_visibility")),
		WithPersonFieldsPageSize(2),
		WithPersonFieldsCursor("c2"),
		WithPersonFieldRequestOptions(pipedrive.WithHeader("X-Test", "1")),
	)
	if err != nil {
...
func TestPersonFieldsService_Create(t *testing.T) {
	t.Parallel()
	...
	field, err := client.PersonFields.Create(
		context.Background(),
		WithPersonFieldName("Priority"),
		WithPersonFieldType(FieldTypeEnum),
		WithPersonFieldDescription("Person priority"),
		WithPersonFieldOptions("High"),
		WithPersonFieldRequestOptions(pipedrive.WithHeader("X-Test", "2")),
	)
	if err != nil {
...
func TestPersonFieldsService_AddOptions(t *testing.T) {
	t.Parallel()
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/personFields/cf_1/options" {
			t.Fatalf("unexpected path: %s", r.URL.Path)
		}
	...
	options, err := client.PersonFields.AddOptions(
		context.Background(),
		"cf_1",
