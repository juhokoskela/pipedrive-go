...
func TestPersonsService_PostalAddressRequests(t *testing.T) {
	t.Parallel()
	address := PersonAddress{Value: "Main Street 1", Country: "FI"}
	for _, method := range []string{http.MethodPost, http.MethodPatch} {
		for _, tt := range []struct {
			...
			{name: "omitted"},
			{name: "empty object", opts: []PersonOption{WithPersonPostalAddress(PersonAddress{})}, want: `{}`},
			{name: "value", opts: []PersonOption{WithPersonPostalAddress(address)}, want: `{"value":"Main Street 1","country":"FI"}`},
			{name: "clear", opts: []PersonOption{ClearPersonPostalAddress()}, want: `null`},
			{name: "clear after value", opts: []PersonOption{WithPersonPostalAddress(address), ClearPersonPostalAddress()}, want: `null`},
			{name: "value after clear", opts: []PersonOption{ClearPersonPostalAddress(), WithPersonPostalAddress(address)}, want: `{"value":"Main Street 1","country":"FI"}`},
		} {
			t.Run(method+"/"+tt.name, func(t *testing.T) {
				client := newTestClient(t, func(w http.ResponseWriter, r *http.Request) {
					...
					if got := string(body["postal_address"]); got != tt.want {
						t.Errorf("postal_address = %s, want %s", got, tt.want)
					}
					w.Header().Set("Content-Type", "application/json")
				...
				if method == http.MethodPost {
					opts := []CreatePersonOption{WithPersonName("Contact")}
					for _, opt := range tt.opts {
						opts = append(opts, opt)
					}
					_, err = client.Persons.Create(t.Context(), opts...)
				} else {
					opts := []UpdatePersonOption{WithPersonName("Contact")}
					for _, opt := range tt.opts {
						opts = append(opts, opt)
					}
					_, err = client.Persons.Update(t.Context(), 1, opts...)
				}
				if err != nil {
					t.Fatal(err)
				}
