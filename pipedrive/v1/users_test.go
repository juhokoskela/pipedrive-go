...
func TestUsersService_List(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "sales", app: UserAccessAppSales},
		{name: "nova", app: UserAccessAppNova},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				w.Header().Set("Content-Type", "application/json")
				_, _ = w.Write([]byte(`{"success":true,"data":[{"id":1,"name":"Jane Doe","email":"jane@example.com","last_login":"2024-01-01 10:00:00","active_flag":true,"access":[{"app":"` + string(tt.app) + `","admin":true,"permission_set_id":"perm-1"}]}]}`))
			}))
			t.Cleanup(srv.Close)
			...
			if len(users[0].Access) != 1 || users[0].Access[0].App != tt.app || users[0].Access[0].PermissionSetID != "perm-1" {
